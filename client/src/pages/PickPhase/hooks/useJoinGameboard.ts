...
export const useJoinGameboard = (setAllChampions: (value: React.SetStateAction<Champion[]>) => void) => {
  const asyncEmit = useAsyncEmits();
  useEffect(() => {
    const joinGameboard = async () => {
      try {
        const response = (await asyncEmit(socket, GAME_ACTIONS.JOIN_GAMEBOARD)) as Record<'champions', Champion[]>;
